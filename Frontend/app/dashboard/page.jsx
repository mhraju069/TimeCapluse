...
const Dashboard = () => {
        ...
        const fetchDashboard = async () => {
            try {
                        ...
                        if (refreshRes.ok) {
                            ...
                            if (retryRes.ok) {
                                ...
                                if (retryData.data.capsule) {
                                    ...
                                    fetchTimelineEvents(retryData.data.capsule.id, refreshData.access);
                                }
                                setLoading(false);
                                return;
                            }
                ...
                if (result.data.capsule) {
                    ...
                    fetchTimelineEvents(result.data.capsule.id, token);
                }
            } catch (err) {
    ...
    const handleSaveCapsule = async (e) => {
        ...
        formData.append('name', capsuleFormData.name);
        formData.append('bio', capsuleFormData.bio);
        formData.append('location', capsuleFormData.location);
        formData.append('dob', capsuleFormData.dob);
        formData.append('is_public', capsuleFormData.is_public);

        if (capsuleProfileFile) {
            formData.append('profile', capsuleProfileFile);
        }
        if (capsuleCoverFile) {
            formData.append('cover', capsuleCoverFile);
        }

        try {
            const res = await fetch(`${API_BASE_URL}/api/capsules/${data.capsule.id}/`, {
                method: 'PATCH',
                headers: {
            ...
            if (!res.ok) throw new Error('Failed to update capsule settings');
            const result = await res.json();
            setData(prev => ({
                ...prev,
                capsule: result
    ...
    const handleUpdateEvent = () => {
        if (data?.capsule?.id) {
            fetchTimelineEvents(data.capsule.id, localStorage.getItem('access_token'));
        }
        setShowModal(false);
    ...
    if (loading) {
        return (
            <div className="dashboard-loading" style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: '#000', color: '#fff' }}>
                <div className="loading-spinner" style={{ width: '40px', height: '40px', border: '3px solid rgba(255,255,255,0.1)', borderTopColor: '#fff', borderRadius: '50%', animation: 'spin 1s linear infinite', marginBottom: '20px' }} />
                <p style={{ fontFamily: 'var(--font-family)', fontSize: '0.95rem', color: 'rgba(255,255,255,0.6)' }}>Curating your digital sanctuary...</p>
    ...
    if (error) {
        return (
            <div className="dashboard-error" style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: '#000', color: '#fff', padding: '20px', textAlign: 'center' }}>
                <div className="error-icon" style={{ fontSize: '3rem', marginBottom: '20px' }}>✦</div>
                ...
                <button onClick={() => window.location.reload()} className="retry-btn" style={{ padding: '10px 24px', background: '#fff', color: '#000', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: '500' }}>Try Again</button>
            </div>
        );
    }
            ...
            <div className="dashboard-content">
                ...
                <header className="dashboard-header">
                    ...
                    <div className="header-actions">
                        {has_capsule ? (
                            <button className="header-btn" onClick={() => router.push(`/capsule/${capsule_id}`)}>
                                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--text-light)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                            ...
                            <button className="header-btn" onClick={() => router.push('/mint')}>
                    ...
                    <section className="dashboard-section capsules-section" style={{ padding: '24px 0 0 0' }}>
                        ...
                        {has_capsule && capsule_id ? (
                            ...
                            <div className="empty-state" style={{ marginTop: '40px' }}>
                                ...
                                <button className="retry-btn" style={{ marginTop: '16px' }} onClick={() => router.push('/mint')}>
                                    Mint Capsule Now
                                </button>
                            </div>
                        )}
