...
const HeartIcon = () => <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-5 w-5 text-white group-hover:text-pink-500 transition-colors"><path d="M19 14c1.49-1.46 3-3.21 3-5.5A5.5 5.5 0 0 0 16.5 3c-1.76 0-3 .5-4.5 2-1.5-1.5-2.74-2-4.5-2A5.5 5.5 0 0 0 2 8.5c0 2.3 1.5 4.05 3 5.5l7 7Z" /></svg>;
...
const GridItem = ({ item }) => {
    return (
        <div className="mb-0 break-inside-avoid relative">
            <img
                src={item.imageUrl}
                alt=""
...
const MasonryGrid = ({ items }) => {
    return (
        <div className="columns-2 gap-0 sm:columns-3 md:columns-4 lg:columns-6 xl:columns-8 2xl:columns-10" style={{ columnWidth: '20px' }}>
            {items.map((item, idx) => (
...
export default function Masonary() {
    ...
    return (
        <div className="font-sans transition-colors relative" style={{ height: '100vh', width: '100%', background: '#000000', overflow: 'hidden', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
            ...
            <div className="absolute inset-0 z-10 pointer-events-none" style={{ background: 'linear-gradient(to left, rgba(0, 0, 0, 0.3) 0%, rgba(0, 0, 0, 0.6) 60%, rgba(0, 0, 0, 0.95) 100%)' }} />
            ...
            <div className="absolute inset-0 z-20 flex flex-col justify-center px-6 md:px-20 py-12 md:py-24 text-white">
                <div className="max-w-3xl relative">
                    ...
                    <div className="absolute -top-[70px] left-[150px] sm:-top-[90px] sm:left-[245px] opacity-95 z-1 pointer-events-none">
                    ...
                    <h1 className="text-white text-5xl sm:text-7xl md:text-8xl font-bold tracking-tight leading-none mb-6" style={{ fontFamily: "'Inter', sans-serif", letterSpacing: '0.1em' }}>
                        RELIC
                    </h1>
                    <p className="text-white/60 italic text-base md:text-xl font-light tracking-wide max-w-xl leading-relaxed" style={{ fontFamily: "var(--font-family)" }}>
                        Relic is a digital home for the moments that matter. <br /> Collect your memories, tell your stories, and <br />keep a piece of your life close — forever.
                    </p>
                    <div className="flex gap-4 mt-8">
                        <button className="px-8 py-3.5 rounded-full border border-white/20 bg-black/40 text-white font-medium hover:bg-white/10 transition-all text-sm md:text-base cursor-pointer" onClick={() => router.push('/capsule')}>
                            CAPSULE
                        </button>
                        <button className="px-8 py-3.5 rounded-full bg-white text-black font-medium hover:bg-white/90 transition-all text-sm md:text-base cursor-pointer" onClick={() => router.push('/mint')}>
                            MINT
                        </button>
                    </div>
                </div>
